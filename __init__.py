...
#        self.complete_request()
    ...
    def st_dirty(self, speech, language):
        if language == 'de-DE':
            self.say("Hummus. Kompost. Bims. Schlamm. Mist. Kies. Erde. Ein Schweinestall, oeffentliche Toiletten, Doenerlaeden")
        elif language == 'fr-FR':
    ...
    @register("de-DE", ".*Wo.*leiche.*verstecken.*"|".*Wo.*toten.*verstecken.*"|".*Wo.*laichen.*verstecken.*")
    ...
    @register("de-DE", ".*deine.*lieblingsfarbe.*")
